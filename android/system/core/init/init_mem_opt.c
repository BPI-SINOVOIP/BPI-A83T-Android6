...
{
#define DISP_SYS_NODE       "/sys/class/script/get_item"
#define SCRIPT_BUF_SIZE     0x10000
    ...
    data = (char *)malloc(SCRIPT_BUF_SIZE);
    if (!data) {
    ...
    /* get lcd_x */
    if (write(fd, "lcd0_para lcd_x", strlen("lcd0_para lcd_x")) < 0) {
        ERROR("%s err: write lcd0_para lcd_x to %s failed\n", __func__, DISP_SYS_NODE);
        goto end;
    }
    lseek(fd, 0L, SEEK_SET);
    if (read(fd, data, SCRIPT_BUF_SIZE) <= 0) {
        lseek(fd, 0L, SEEK_SET);
        if (write(fd, "lcd0 lcd_x", strlen("lcd0 lcd_x")) < 0) {
            ERROR("%s err: write lcd0 lcd_x to %s failed\n", __func__, DISP_SYS_NODE);
            goto end;
        }
        lseek(fd, 0L, SEEK_SET);
        if (read(fd, data, SCRIPT_BUF_SIZE) <= 0) {
            ERROR("%s err: read %s failed\n", __func__, DISP_SYS_NODE);
            goto end;
        }
    }
    *width = *(uint32_t *)(data);
    ...
    /* get lcd_y */
    lseek(fd, 0L, SEEK_SET);
    if (write(fd, "lcd0_para lcd_y", strlen("lcd0_para lcd_y")) < 0) {
        ERROR("%s err: write lcd0_para lcd_y to %s failed\n", __func__, DISP_SYS_NODE);
        goto end;
    }
    lseek(fd, 0L, SEEK_SET);
    if (read(fd, data, SCRIPT_BUF_SIZE) <= 0) {
        lseek(fd, 0L, SEEK_SET);
        if (write(fd, "lcd0 lcd_y", strlen("lcd0 lcd_y")) < 0) {
            ERROR("%s err: write lcd0 lcd_y to %s failed\n", __func__, DISP_SYS_NODE);
            goto end;
        }
        lseek(fd, 0L, SEEK_SET);
        if (read(fd, data, SCRIPT_BUF_SIZE) <= 0) {
